#!/usr/bin/env python
"""
Rebuild Extraction Cache for DuRecDial Experiments.

This script pre-computes and caches the extraction phase for all DuRecDial
conversations, so that subsequent experiments can skip the expensive
LLM extraction calls.

Usage:
    # Rebuild cache for all configs
    python -m experiments.rebuild_durecdial_cache

    # Rebuild cache for specific config
    python -m experiments.rebuild_durecdial_cache --config no_vage

    # Force rebuild (clear existing cache first)
    python -m experiments.rebuild_durecdial_cache --force

    # Rebuild with custom dataset
    python -m experiments.rebuild_durecdial_cache -d experiments/data/durecdial_dev_en.jsonl
...
def main():
    parser = argparse.ArgumentParser(
        description="Rebuild extraction cache for DuRecDial experiments"
    )
    parser.add_argument(
        "--dataset",
        "-d",
        default="experiments/data/durecdial_test.jsonl", # Placeholder for actual dataset
        help="Path to DuRecDial JSONL dataset",
    )
    parser.add_argument(
        "--min-turns",
        type=int,
        default=30,
        help="Minimum number of turns for conversations to be included",
    )
    parser.add_argument(
        "--config",
        "-c",
    ...
    from experiments.durecdial_adapter import load_durecdial_file, convert_durecdial_to_locomo
    from experiments.agents.cogcanvas_agent import CogCanvasAgent

    # Load dataset
    print(f"Loading DuRecDial dataset from {args.dataset} (min_turns={args.min_turns})...")
    raw_data = load_durecdial_file(args.dataset)
    conversations = convert_durecdial_to_locomo(raw_data, min_turns=args.min_turns)
    ...
    if args.force:
        for config_name in configs_to_rebuild:
            ...
            # When forcing clear, ensure we target the DuRecDial cache specifically if needed
            # For now, it clears based on ExtractionConfig hash, which is generic.
    ...
    for config_name in configs_to_rebuild:
        ...
        def process_conv(conv):
            try:
                ...
                # DuRecDial conversations already converted to LoMoCoConversation, so this method exists
