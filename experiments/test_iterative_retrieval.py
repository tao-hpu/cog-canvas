...
def test_iterative_retrieval(num_samples=20):
    ...
    import os
    canvas = Canvas(
        extractor_model=os.getenv("MODEL_WEAK_2", "gpt-4o-mini"),
    ...
    for i, qa in enumerate(questions):
        ...
        result_single = canvas.retrieve(query, top_k=15, method="hybrid", include_related=True)
        single_context = canvas.inject(result_single, format="compact", max_tokens=2000)

        # Iterative retrieval: Round 1 + Round 2
        result_r1 = canvas.retrieve(query, top_k=10, method="hybrid", include_related=True)
        if result_r1.objects:
            # Expand query with top object's content
            expanded_query = f"{query} {result_r1.objects[0].content}"
            result_r2 = canvas.retrieve(expanded_query, top_k=10, method="hybrid", include_related=True)
        ...
        iterative_context = canvas.inject(result_iterative, format="compact", max_tokens=2000)
        ...
        single_overlap = len(expected_keywords & single_keywords) / max(len(expected_keywords), 1)
        iterative_overlap = len(expected_keywords & iterative_keywords) / max(len(expected_keywords), 1)
        ...
        if abs(single_overlap - iterative_overlap) > 0.1:
            print(f"\n[{i+1}] Q: {query[:60]}...")
            print(f"    Single: {single_overlap:.1%} | Iterative: {iterative_overlap:.1%}")
            if iterative_overlap > single_overlap:
                print(f"    ✅ Iterative better by {(iterative_overlap - single_overlap)*100:.1f}pp")
            else:
                print(f"    ❌ Single better by {(single_overlap - iterative_overlap)*100:.1f}pp")
    ...
    print(f"Difference:                          {(avg_iterative - avg_single)*100:+.1f}pp")
